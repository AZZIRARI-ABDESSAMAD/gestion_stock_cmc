...
@section('title', 'Historique Global des Commandes')

@section('content')
<div class="space-y-6 animate-fade-in">
    <div>
        <h2 class="text-2xl font-bold text-slate-800">Historique Global des Commandes</h2>
        <p class="text-sm text-slate-500">Consultez l'historique complet de toutes les commandes traitées.</p>
    </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50 border-b border-slate-200 text-slate-500 text-xs uppercase font-bold tracking-wider">
                        ...
                        <th class="px-6 py-4">Date de Traitement</th>
                        <th class="px-6 py-4">Demandeur / Pôle</th>
                        <th class="px-6 py-4">Statut</th>
                        <th class="px-6 py-4">Articles & Quantités (Demandé | Validé)</th>
                    </tr>
                </thead>
                    ...
                    @else
                        @foreach($commandes as $commande)
                            <tr class="hover:bg-slate-50/40 transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    <span class="font-semibold text-slate-700 block">{{ $commande->user->name }}</span>
                                    <span class="text-xs text-slate-400">Pôle : {{ $commande->user->space->name_espace ?? 'N/A' }}</span>
                                </td>
                                <td class="px-6 py-4">
                                    ...
                                    <div class="space-y-1 max-w-md">
                                        @foreach($commande->products as $product)
                                            <div class="flex items-center justify-between text-xs border-b border-slate-100 pb-1 last:border-0 last:pb-0">
                                                <span class="text-slate-600 font-medium">{{ $product->title }}</span>
